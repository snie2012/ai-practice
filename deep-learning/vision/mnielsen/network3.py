...
# load the MNIST data
def load_data_shared(filename='data/mninst.pkl.gz'):
...
class Network(object):
	...
	def SGD(self, training_data, epochs, mini_batch_size, eta, validation_data, test_data, lmbda=0.0):
		...
		# define the regularied cost function, symbolic gradients, and updates
		...
		for epoch in xrange(epochs):
			for minibatch_index in xrange(num_training_batches):
				...
				if validation_accuracy >= best_validation_accuracy:
					print "This is the best validation accuracey to date."
...
# Define different types of layers. Conform to a common interface.
class ConvPoolLayer(object):
	"""
	Used to create a combination of a convolutional and a max-pooling layer.
	A more sophisticated implementation would separate the two, but for our 
	purposes we'll always use them together, and it simplifies the code, so
	...
	def __init__(self, filter_shape, image_shape, poolsize=(2, 2), activation_fn=sigmoid):
		...
		filters, the number of input feature maps (if the input layer is the first
		layer, then it is the nubmer of input channels such as r, g, and b), 
		the filter height, and the filter width.

		`image_shape` is a tuple of length 4, whose entries are the mini-batch
		size, the number of input feature maps, the image height, and the image
		width

		`poolsize` is a tuple of length 2, whose entries are the y and x pooling 
		sizes.
		"""
	...
	def set_inpt(self, inpt, inpt_dropout, mini_batch_size):
		...
		self.output_dropout = self.output # no dropout in the convolutional layer
...
class SoftmaxLayer(object):
	...
	def cost(self, net):
		...
		return -T.mean(T.log(self.output_dropout)[T.arrage(net.y.shape[0]), net.y])
