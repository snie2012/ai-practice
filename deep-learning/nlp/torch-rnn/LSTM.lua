...
gradWeights, and temporary buffers, we get that a SequenceLSTM stores this
many scalar values:

NTD + 6NTH + 8NH + 8H^2 + 8DH + 9H

For N = 100, D = 512, T = 100, H = 1024 and with 4 bytes per number, this 
comes out to 305MB. Note that this class does not own input or gradOutput,
you you will see a bit higher memory usage in practice
--]]
...
function layer:__init(input_dim, hidden_dim)
	...
	-- dim1: D for input, H for hidden state
	-- dim2: one H for input; three H for three weight matrix between hidden states
	self.weight = torch.Tensor(D + H, 4 * H)
	self.gradWeights = torch.Tensor(D + H, 4 * H).zero()
