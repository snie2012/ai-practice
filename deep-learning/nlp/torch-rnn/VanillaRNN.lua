--[[
Following code from:
https://github.com/jcjohnson/torch-rnn/blob/master/VanillaRNN.lua
--]]
...
--[[
Vanalla RNN with tanh nonlinearity that operates on entire sequences of data.

The RNN has an input dim of D, a hidden dim of H, operates over sequences of length T
and minibatches of size N.
...
SequenceRNN_TN swaps th order of time and minibatch dimensions; this is very slightly
faster, but probably not worth it since it is more irritating to work with.
--]]
...
function layer:updateOutput(input)
	...
	if not h0 then
		h0 = self.h0
		--[[if `self.remember_states` if false, initialize `h0` to be zeros]]
		if h0:nElement() == 0 or not self.remember_states then
			h0:resize(N, H):zero()
		--[[otherwise, initialize `h0` to be the values of the final hidden state from 
		the previous call]]
		elseif self.remember_states then
			local prev_N, prev_T = self.output:size(1), self.output:size(2)
			assert(prev_N == N, 'batch size must to constant to remember states')
			h0:copy(self.output[{{}, prev_T}])
		end
	end

	local bias_expand = self.bias:view(1, H):expand(N, H) --[[expand bias for the calculation of the entire minibatch in one pass]]
	...
	for t = 1, T do
		...
		--[[In torch documentation, it looks like `:addmm` does not accumulate the result.
...
--[[
Normally we do not implement backward, and instead just implement updateGradInput and 
accGradParameters. However for an RNN, separating these two operations would result in
quite a bit of repeated code and computation. Therefore, we will just implement backward 
and update gradInput and gradients with respect to parameters at the same time
--]]
